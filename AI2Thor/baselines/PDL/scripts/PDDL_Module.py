...
import difflib
import sys
# Ensure local resources/ is importable when running from repo root
...
from MultiRobotExecutor import MultiRobotExecutor
...
class LLMHandler:
    ...
    def setup_api(self, api_key_file: str) -> None:
        ...
        try:
            # Try direct paths first, then fallback to PDL folder
            candidates = [
...
class TaskManager:
    ...
    def process_tasks(self, test_tasks: List[str], robot_ids: List[List[int]], objects_ai: str,
                       floor_plan: Optional[int] = None) -> None:
        """
        TaskManager 전체 파이프라인의 main함수
        test_tasks -> 자연어 task 리스트
        robot_ids -> 로봇 ID들
        objects_ai -> 추출한 오브젝트 이름과 무게
        floor_plan -> AI2-THOR FloorPlan 번호 (거리 기반 LP 최적화 시 사용, None이면 거리 비활성)
        """
        try:
