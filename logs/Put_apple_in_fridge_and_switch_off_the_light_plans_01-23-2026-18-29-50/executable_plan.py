...
def execute_task(robots):
    # Complete plan execution for: Put apple in fridge and switch off the light
    def robot1_actions():
    ...
    t1 = threading.Thread(target=robot1_actions)
    t2 = threading.Thread(target=robot2_actions)
...
task_thread = threading.Thread(target=execute_task, args=(robots,))
task_thread.start()
task_thread.join()
